...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  loginBtn.addEventListener('click',()=>{
    ...
    currentUser=user; state=users[user].state; localStorage.setItem(STORAGE_KEY,JSON.stringify(users));
    loginScreen.classList.add('hidden'); appScreen.classList.remove('hidden');
    render();
  });

  logoutBtn.addEventListener('click',()=>{
    saveUser(); currentUser=null; state={ presets:[], entries:{} };
  ...
  function render(){
    ...
    list.forEach(item=>{
      const li=document.createElement('li'); li.textContent=`${item.title} (${item.points} pts)`; entriesList.appendChild(li);
    });
  ...
  function renderChart(){
    ...
    chart=new Chart(ctx,{ type:'bar', data:{ labels, datasets:[{ label:'Pontos', data, backgroundColor:'rgba(99,102,241,0.8)' }] }, options:{ responsive:true, scales:{y:{beginAtZero:true}} } });
  }
